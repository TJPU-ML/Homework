...
class LogisticRegression:
    ...
    def feature_engineering(self, datas, train_prob):
        # 将花的类型转换为0，1，2
        class_mapping = {label: idx for idx, label in enumerate(np.unique(datas['Name']))}
    ...
    def predict(self, x_test, y_test):
        ...
        test_x_1 = test_x_1.reshape(int(self.sample * np.float32(1 - self.train_prob)), 1)
        test_x_2 = test_x_2.reshape(int(self.sample * np.float32(1 - self.train_prob)), 1)

        # 由于参数是根据训练集训练得到的，故shape不匹配测试集，故需要将参数的shape调整至符合测试集
        index = list(range(int(self.sample * np.float32(1 - self.train_prob)), int(self.sample * self.train_prob)))

        theta_0 = np.delete(self.theta_0, index)
        theta_1 = np.delete(self.theta_1, index)
        theta_2 = np.delete(self.theta_2, index)

        theta_0 = theta_0.reshape(int(self.sample * np.float32(1 - self.train_prob)), 1)
        theta_1 = theta_1.reshape(int(self.sample * np.float32(1 - self.train_prob)), 1)
        theta_2 = theta_2.reshape(int(self.sample * np.float32(1 - self.train_prob)), 1)
        ...
        for val in y_pred:
            if (val >= 0.5):
                ...
                y_pred_1.append(0)
        print('The accuracy of the score is: ', accuracy_score(y_test, y_pred_1))
    ...
    def plot_decision_region(self, feature, label):
        ...
        ax.plot(x, y)
        ax.legend(labels=['fitting-curve','Iris-setosa', 'Iris-versicolor'], loc='best')
...
if __name__ == "__main__":
    ...
    x_1, x_2, y_train, x_test, y_test = lr.feature_engineering(iris, train_prob=0.9)    #数据预处理
